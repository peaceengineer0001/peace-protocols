...
TOOLS: List[Dict[str, Any]] = [
    {"name": "text_to_cad", "description": "Generate parametric CAD from text", "inputSchema": {"type": "object"}},
    {"name": "image_to_cad", "description": "Generate CAD from an image", "inputSchema": {"type": "object"}},
    {"name": "export_model", "description": "Export STL/SCAD/DXF", "inputSchema": {"type": "object"}},
]
...
class CadamAdapter:
    """Wrap CADAM as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("cadam: adapter not connected")
        raise NotImplementedError(
            "cadam.call_tool('%s') requires the live upstream service. "
            "See integrations/cadam/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "CadamAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return CadamAdapter(spec, **kwargs)

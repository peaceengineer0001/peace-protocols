...
TOOLS: List[Dict[str, Any]] = [
    {"name": "reconstruct", "description": "Reconstruct a model from an image", "inputSchema": {"type": "object"}},
    {"name": "refine_pass", "description": "Run one sculpting pass", "inputSchema": {"type": "object"}},
    {"name": "export_ts", "description": "Export Three.js/TS code", "inputSchema": {"type": "object"}},
]
...
class Img2threejsAdapter:
    """Wrap img2threejs as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("img2threejs: adapter not connected")
        raise NotImplementedError(
            "img2threejs.call_tool('%s') requires the live upstream service. "
            "See integrations/img2threejs/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "Img2threejsAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return Img2threejsAdapter(spec, **kwargs)

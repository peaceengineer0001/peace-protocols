...
TOOLS: List[Dict[str, Any]] = [
    {"name": "create_room", "description": "Create a conference room", "inputSchema": {"type": "object"}},
    {"name": "schedule_meeting", "description": "Schedule a meeting", "inputSchema": {"type": "object"}},
    {"name": "end_room", "description": "End a room", "inputSchema": {"type": "object"}},
]
...
class HivetalkSfuAdapter:
    """Wrap HiveTalk SFU as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("hivetalk-sfu: adapter not connected")
        raise NotImplementedError(
            "hivetalk-sfu.call_tool('%s') requires the live upstream service. "
            "See integrations/hivetalk_sfu/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "HivetalkSfuAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return HivetalkSfuAdapter(spec, **kwargs)

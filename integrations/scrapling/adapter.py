...
TOOLS: List[Dict[str, Any]] = [
    {"name": "scrape", "description": "Adaptive extract targeted content", "inputSchema": {"type": "object"}},
    {"name": "crawl", "description": "Concurrent spider crawl", "inputSchema": {"type": "object"}},
]
...
class ScraplingAdapter:
    """Wrap Scrapling as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("scrapling: adapter not connected")
        raise NotImplementedError(
            "scrapling.call_tool('%s') requires the live upstream service. "
            "See integrations/scrapling/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "ScraplingAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return ScraplingAdapter(spec, **kwargs)

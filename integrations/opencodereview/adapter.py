"""
MCP adapter for Alibaba OpenCodeReview
======================================
...
TOOLS: List[Dict[str, Any]] = [
    {"name": "review_diff", "description": "Review a diff", "inputSchema": {"type": "object"}},
    {"name": "review_pr", "description": "Review a pull request", "inputSchema": {"type": "object"}},
    {"name": "review_files", "description": "Review specific files", "inputSchema": {"type": "object"}},
]
...
class OpencodereviewAdapter:
    """Wrap Alibaba OpenCodeReview as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("opencodereview: adapter not connected")
        raise NotImplementedError(
            "opencodereview.call_tool('%s') requires the live upstream service. "
            "See integrations/opencodereview/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "OpencodereviewAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return OpencodereviewAdapter(spec, **kwargs)

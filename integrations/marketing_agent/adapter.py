"""
MCP adapter for Marketing Agent Framework
=========================================
...
TOOLS: List[Dict[str, Any]] = [
    {"name": "build_campaign", "description": "Build a marketing campaign", "inputSchema": {"type": "object"}},
    {"name": "generate_content", "description": "Generate content assets", "inputSchema": {"type": "object"}},
    {"name": "plan_distribution", "description": "Plan channel distribution", "inputSchema": {"type": "object"}},
]
...
class MarketingAgentAdapter:
    """Wrap Marketing Agent Framework as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("marketing-agent: adapter not connected")
        raise NotImplementedError(
            "marketing-agent.call_tool('%s') requires the live upstream service. "
            "See integrations/marketing_agent/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "MarketingAgentAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return MarketingAgentAdapter(spec, **kwargs)

...
TOOLS: List[Dict[str, Any]] = [
    {"name": "start_share", "description": "Start a screen share", "inputSchema": {"type": "object"}},
    {"name": "join_share", "description": "Join a share", "inputSchema": {"type": "object"}},
    {"name": "stop_share", "description": "Stop sharing", "inputSchema": {"type": "object"}},
]
...
class BananasAdapter:
    """Wrap Bananas as an MCP server on the Peace Protocols bus."""
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]) -> Any:
        if not self._connected:
            raise RuntimeError("bananas: adapter not connected")
        raise NotImplementedError(
            "bananas.call_tool('%s') requires the live upstream service. "
            "See integrations/bananas/README.md for setup." % name
        )
...
def build(spec: Any = None, **kwargs: Any) -> "BananasAdapter":
    """Factory used by the connection pool's ``client_factory``."""
    return BananasAdapter(spec, **kwargs)
